...
const login = async () => {
  ...
  try {
    ...
    const code = error.code || error.response?.data?.message || 'Errore generico'
    errorMessage.value = code === 'auth/invalid-credential' ? 'Credenziali non valide' : `${code}`
  }
}
</script>
...
const login = () => {
  errorMessage.value = ''; // Reset del messaggio di errore

  signInWithEmailAndPassword(getAuth(), email.value, password.value)
    .then((data) => {
      console.log("Login avvenuta!");

      // Quando il login è riuscito, settiamo l'utente nello store
      const user = data.user; // Prendi l'oggetto dell'utente da Firebase
      userStore.login({
        email: user.email || '',
        uid: user.uid,
